...
const MyAddedVisas = () => {
  ...
  const handleDelete = async (id) => {
    ...
    const res = await fetch(`http://localhost:5000/myvisas/${id}`, 
        { 
            method: 'DELETE' 
        });
    if (res.ok) setVisas(visas.filter(v => v._id !== id));
  };
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
        {visas.map(visa => (
          <div key={visa._id} className="card bg-white shadow-xl p-4 rounded-xl">
            <img src={visa.countryImage} alt={visa.countryName} className="h-40 w-full object-cover rounded-lg mb-3"/>
        ...
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center">
          <div className="bg-white p-6 rounded-xl shadow-xl w-96">
            ...
            <form onSubmit={handleUpdate} className="space-y-3">
              ...
              <input name="fee" placeholder="EnterFee " defaultValue={editingVisa.fee} className="input input-bordered w-full" required />
